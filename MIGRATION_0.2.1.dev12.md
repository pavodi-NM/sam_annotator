...
**Important:** v0.2.1.dev11 had a critical missing dependency issue and should not be used. Please upgrade to v0.2.1.dev12.

## Bug Fixes

### Critical: Missing Dependency (Fixed in dev12)

**Issue:** v0.2.1.dev11 was missing the `segment-anything` dependency, causing import errors.

**Fix:** Re-added `segment-anything>=1.0` to dependencies in v0.2.1.dev12.

### Critical: Model Type Parameter Propagation

**Issue:** In versions prior to 0.2.1.dev12, the `--model_type` parameter was not correctly passed to the SAM predictor, causing model architecture mismatches with checkpoint files.
